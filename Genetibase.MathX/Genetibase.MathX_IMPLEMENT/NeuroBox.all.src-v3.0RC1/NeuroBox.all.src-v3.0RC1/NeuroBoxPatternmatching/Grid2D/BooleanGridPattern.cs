...
namespace NeuroBox.PatternMatching.Grid2D
{
    ...
    /// </summary>
    [Serializable]
    public class BooleanGrid2DPattern : BooleanPattern
    {
        private int _width;
        private int _height;
        ...
        public BooleanGrid2DPattern(string title, int width, int height, bool[] input, int numberOfClasses, int classification)
            : base(title, input, numberOfClasses, classification)
        {
            this._width = width;
            this._height = height;
        }
