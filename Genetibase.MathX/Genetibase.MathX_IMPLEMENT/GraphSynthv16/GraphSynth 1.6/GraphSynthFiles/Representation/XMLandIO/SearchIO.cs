...
namespace GraphSynth.Representation
{
    public static class SearchIO
    {
        ...
        static string defaultMiscObject = "misc";
        static Hashtable miscHash = new Hashtable();
        public static object miscObject
        {
            set
            {
                string searchThreadName = Thread.CurrentThread.Name;
                if (searchThreadName == null) defaultMiscObject = value.ToString();
                else if (miscHash.ContainsKey(searchThreadName))
                    miscHash[searchThreadName] = value;
                else miscHash.Add(searchThreadName, value);
            }
            get
            {
                if (Thread.CurrentThread.Name == null) return defaultMiscObject;
                else return getMiscObject(Thread.CurrentThread.Name);
            }
        }
        ...
        public static void output(object message, int verbosityLimit)
        {
            ...
            //SafeInvokeHelper.SearchIO(Thread.CurrentThread.Name + message.ToString());
        }
    ...
    public class SearchIOToTextWriter : System.IO.TextWriter
    {
        public RichTextBox outputBox;

        public override void WriteLine(string str)
        {
            SafeInvokeHelper.Invoke(outputBox, "AppendText", Thread.CurrentThread.Name + str + "\n");
        }
