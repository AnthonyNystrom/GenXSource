...
#Region "Tab strip renderer"
    Private Class TabStripRenderer
        Inherits ToolStripSystemRenderer

        Private m_pageViewer As DiagramPageViewer
        ...
        Protected Overrides Sub OnRenderToolStripBorder(ByVal e As System.Windows.Forms.ToolStripRenderEventArgs)
            ...
            With e.Graphics
                Using p As New Pen(SystemColors.Window)
                    .DrawLine(p, .ClipBounds.Left, .ClipBounds.Top, .ClipBounds.Right, .ClipBounds.Top)
                End Using
            End With
        End Sub
        ...
        Protected Overrides Sub OnRenderLabelBackground(ByVal e As System.Windows.Forms.ToolStripItemRenderEventArgs)
            With e.Graphics
                If e.Item.Tag Is m_pageViewer.CurrentPage Then
                    Using b As New SolidBrush(SystemColors.Window)
                        .FillRectangle(b, .ClipBounds)
                    End Using
                    Using p As New Pen(SystemColors.ButtonShadow)
                        .DrawLine( _
                            p, _
                            .VisibleClipBounds.Left, .VisibleClipBounds.Top, _
                            .VisibleClipBounds.Left, .VisibleClipBounds.Bottom - 2 _
                        )
                        .DrawLine( _
                            p, _
                            .VisibleClipBounds.Right, .VisibleClipBounds.Top, _
                            .VisibleClipBounds.Right, .VisibleClipBounds.Bottom - 2 _
                        )
                        .DrawLine( _
                            p, _
                            .VisibleClipBounds.Left, .VisibleClipBounds.Bottom - 1, _
                            .VisibleClipBounds.Right, .VisibleClipBounds.Bottom - 1 _
                        )

                    End Using
                Else
                    Using p As New Pen(SystemColors.InactiveBorder)
                        .DrawLine( _
                            p, _
                            .VisibleClipBounds.Left, .VisibleClipBounds.Top + 3, _
                            .VisibleClipBounds.Left, .VisibleClipBounds.Bottom - 4 _
                        )
                    End Using
                End If
            End With
        ...
        Friend Sub New(ByVal viewer As DiagramPageViewer)
            m_pageViewer = viewer
        End Sub
    End Class
#End Region
...
#Region " Page management "
    ...
    Private Sub SelectPage(ByVal page As Page)
        With CurrentPageViewer
            .Page = page
            '.Invalidate()
            PopulatePageExplorer()
        End With
        For Each pageTab As ToolStripItem In PageSelector.Items
            If pageTab.Tag Is page Then
                pageTab.BackColor = SystemColors.Window
            Else
                pageTab.BackColor = SystemColors.Control
            End If
        Next
    ...
    Private Sub PopulatePageSelector()
        ...
        For Each page As Page In m_diagram.Pages
            Dim pageTab As New ToolStripLabel(page.Name)
            With pageTab
...
#Region " Explorer Management Events "
    ...
    Private Sub Explorers_Selected(ByVal sender As Object, ByVal e As System.Windows.Forms.TabControlEventArgs) Handles Explorers.Selected
        ManageExplorers()
    End Sub
...
#End Region
    ...
    Public Sub New()
        ...
        PageSelector.Renderer = New TabStripRenderer(Me)
    End Sub
End Class
