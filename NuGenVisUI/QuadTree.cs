...
namespace Genetibase.VisUI.UI
{
    ...
    public class QuadTree<T> : QuadTreeNode<T> where T : IQuadTreeItem
    {
        uint itemsCount;
    ...
    public class QuadTreeNode<T> where T : IQuadTreeItem
    {
        ...
        public QuadTreeNode(Point position, Size dimensions)
        {
            ...
            positionUpper = position + dimensions;
        ...
        {
            dimensions = new Size(parent.dimensions.Width / 2, parent.dimensions.Height / 2);
            switch (subNodeNum)
            {
                case 0:
                    ...
                    break;
                case 1:
                    position = parent.position + dimensions;
                    break;
                case 2:
                    position = parent.position;
                    break;
        ...
        {
            // find item the easy way :)
            // TODO: Update to locate item by location..
            return RemoveFromNode(item, this);
        }

        private static bool RemoveFromNode(T item, QuadTreeNode<T> node)
        {
            bool result = false;
            if (node != null)
            {
                if (node.nodeItems != null)
                {
                    if (node.nodeItems.Remove(item))
                        result = true;
                }
                // check sub-nodes
                if (!result && node.subNodes != null)
                {
                    foreach (QuadTreeNode<T> subNode in node.subNodes)
                    {
                        if (RemoveFromNode(item, subNode))
                        {
                            result = true;
                            break;
                        }
