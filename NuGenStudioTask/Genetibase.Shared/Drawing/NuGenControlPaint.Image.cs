...
namespace Genetibase.Shared.Drawing
{
	partial class NuGenControlPaint
	{
		...
		/// <paramref name="sourceBitmap"/> is <see langword="null"/>.
		/// </exception>
		public static Bitmap GetMonochromeBitmap(Bitmap sourceBitmap)
		{
			if (sourceBitmap == null)
			{
				throw new ArgumentNullException("bmp");
		...
		/// </exception>
		public static Bitmap GetThumbnail(Image sourceImage, Size thumbnailSize)
		{
			if (sourceImage == null)
			{
			...
			if (thumbnailSize.Height < 1 || thumbnailSize.Width < 1)
			{
				throw new ArgumentException(
					string.Format(Properties.Resources.Argument_InvalidResolutionStruct, "thumbnailSize")
				);
			}

			Bitmap bmp = new Bitmap(thumbnailSize.Width, thumbnailSize.Height);
		...
		public static void GetThumbnail(Image sourceImage, Size thumbnailSize, string outputPath)
		{
			...
			using (Bitmap bmp = NuGenControlPaint.GetThumbnail(sourceImage, thumbnailSize))
			{
				bmp.Save(outputPath, jpgCodecInfo, encoderParams);
			}
		}
		...
		public static Rectangle ImageBoundsFromContentAlignment(Size imageSize, Rectangle fitRectangle, ContentAlignment contentAlignment)
		{
			...
			switch (contentAlignment)
			{
				...
				{
					cx = fitRectangle.Left + fitRectangle.Width / 2 - imageSize.Width / 2;
				...
				{
					cx = fitRectangle.Left;
					cy = fitRectangle.Top + fitRectangle.Height / 2 - imageSize.Height / 2;
		...
		/// <summary>
		/// Scales the specified <see cref="Image"/> to fit the specified <see cref="Rectangle"/> and
		/// returns <see cref="Rectangle"/> to specify the image bounds.
		/// </summary>
		/// <exception cref="T:System.ArgumentNullException">
		/// <paramref name="img"/> is <see lagnword="null"/>.
		/// </exception>
		public static Rectangle ScaleToFit(Rectangle targetRect, Image imageToScale)
		{
			if (!targetRect.IsEmpty)
			{
				if (imageToScale == null)
				{
					throw new ArgumentNullException("imageToScale");
				}

				Rectangle resultRect = new Rectangle(targetRect.Location, targetRect.Size);

				if (resultRect.Height * imageToScale.Width > resultRect.Width * imageToScale.Height)
				{
					resultRect.Height = resultRect.Width * imageToScale.Height / imageToScale.Width;
					resultRect.Y += (targetRect.Height - resultRect.Height) / 2;
				}
				else
				{
					resultRect.Width = resultRect.Height * imageToScale.Width / imageToScale.Height;
