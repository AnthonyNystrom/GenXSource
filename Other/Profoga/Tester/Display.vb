...
Public Class Display
    ...
    Private aminos As Generic.Dictionary(Of Integer, Aminoacids.Molecule)
    Private Loaded As Boolean = False
    Private AminoAcidNumber As Integer = 2
    ...
    Private Sub OpenToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OpenToolStripMenuItem.Click
        ...
        aminos = New Generic.Dictionary(Of Integer, Aminoacids.Molecule)
        For i As Integer = 0 To AminoAcidNumber - 1
            If dlgOpenAmino.ShowDialog = Windows.Forms.DialogResult.OK Then
                filename = dlgOpenAmino.FileName
            Else
                Exit Sub
            End If

            Dim oneAmino As Aminoacids.Molecule
            'oneAmino.Load(filename)

            Dim ser As New Xml.Serialization.XmlSerializer(GetType(Aminoacids.Molecule), "http://www.xml-cml.org/schema/cml2/core")
            Dim aMoleculeStream As New IO.StreamReader(filename)
            ...
            aminos.Add(oneAmino.Id, oneAmino)
            If i >= 1 Then Aminoacids.Molecule.Join(aminos(oneAmino.Id - 1), oneAmino)
        Next
                ...
                If ((oneAtom.Z >= (z / 10)) AndAlso (oneAtom.Z < ((z + 1) / 10))) Then
                    ...
                    For Each aBond As Aminoacids.Bond In oneAtom.Bonds
                        ...
                        If aBond.Order <> Aminoacids.BondOrder.Peptide Then
                            targetAtom = aminos(oneAtom.MoleculeId).Atoms(aBond.LinkAtom)
                        Else
                            With aminos(aBond.LinkMolecule)
        ...
        If Loaded AndAlso Me.Moving Then
            Select Case e.Button
                ...
                Case Windows.Forms.MouseButtons.Right
                    ...
                    aminos(1).RotateAngC(s * Math.PI / 36)
                Case Windows.Forms.MouseButtons.Middle
                    s = Math.Sign(Me.ddX - e.X)
                    aminos(2).RotateAngN(s * Math.PI / 36)
                    'For Each oneAmino As Aminoacids.Molecule In aminos.Values
