...
{
	((EncoderObjectWM*)(cinfo->client_data))->currentFile->Seek(0, File::ESO_START);
}
...
static	boolean	FillInputBufferForFile ( j_decompress_ptr cinfo)
{
	...
	nbytes = ((EncoderObjectWM*)(cinfo->client_data))->currentFile->Read(((EncoderObjectWM*)(cinfo->client_data))->innerBuffer, INNER_BUFFER_SIZE);
	if (nbytes <= 0) 
	{
	...
	src->next_input_byte = ((EncoderObjectWM*)(cinfo->client_data))->innerBuffer;
...
{
	((EncoderObjectWM*)(cinfo->client_data))->isOk = false;
	cinfo->err->isError = true;
}


EncoderObjectWM::EncoderObjectWM( /*EncoderObjectListener *aListener*/ )
...
EncoderObjectWM::~EncoderObjectWM()
{
...
const ImageInfo * EncoderObjectWM::GetInfo( char8 *dataBuffer, int32 bufferSize ) /*= 0*/
{
...
const ImageInfo * EncoderObjectWM::GetInfo( File *file ) /*= 0*/
{
...
bool EncoderObjectWM::GetSurface( char8 *srcBuffer, int32 bufferSize, GraphicsSystem::Surface *destSurface, ImageInfo *pDestProps /*= NULL*/ ) /*= 0*/
{
	if (state != EES_IDLE)
	{
		return false;
...
bool EncoderObjectWM::GetSurface( File *srcFile, GraphicsSystem::Surface *destSurface, ImageInfo *pDestProps /*= NULL*/ ) /*= 0*/
{
	if (state != EES_IDLE)
	{
		return false;
...
bool EncoderObjectWM::GetData( GraphicsSystem::Surface *sourceSurface, char8 *destBuffer, int32 bufferSize ) /*= 0*/
{
...
void EncoderObjectWM::LoadData(GraphicsSystem::Surface *destSurface, ImageInfo *img)
{
...
void EncoderObjectWM::DecompressLines()
{
	GraphicsSystem *gr = GetApplication()->GetGraphicsSystem();
...
void EncoderObjectWM::SetSourceToBuffer( char8 *buffer, int32 size )
{
	isOk = true;
...
void EncoderObjectWM::SetSourceToFile( File *file )
{
	isOk = true;
...
bool EncoderObjectWM::IsFree()
{
	return state == EES_IDLE;
}

bool EncoderObjectWM::Resize( File *srcFile, char8 *destBuffer, int32 bufferSize, ImageInfo *pDestProps )
{
	if (state != EES_IDLE)
	{
		return false;
...
void EncoderObjectWM::ResizeLines()
{
