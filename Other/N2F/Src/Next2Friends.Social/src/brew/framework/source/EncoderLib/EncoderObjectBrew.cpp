...
{
	((EncoderObjectBrew*)(cinfo->client_data))->currentFile->Seek(0, File::ESO_START);
}
...
static	boolean	FillInputBufferForFile ( j_decompress_ptr cinfo)
{
	...
	nbytes = ((EncoderObjectBrew*)(cinfo->client_data))->currentFile->Read(((EncoderObjectBrew*)(cinfo->client_data))->innerBuffer, INNER_BUFFER_SIZE);
	if (nbytes <= 0) 
	{
	...
	src->next_input_byte = ((EncoderObjectBrew*)(cinfo->client_data))->innerBuffer;
...
static	void	SkipInputDataForFile ( j_decompress_ptr cinfo, long count )
{
	...
	*/
	if (count > 0) 
	{
		...
		src->next_input_byte += (size_t) count;
		src->bytes_in_buffer -= (size_t) count;
	}
}
...
{
	((EncoderObjectBrew*)(cinfo->client_data))->isOk = false;
	cinfo->err->isError = true;
}


EncoderObjectBrew::EncoderObjectBrew( /*EncoderObjectListener *aListener*/ )
...
EncoderObjectBrew::~EncoderObjectBrew()
{
...
const ImageInfo * EncoderObjectBrew::GetInfo( char8 *dataBuffer, int32 bufferSize ) /*= 0*/
{
...
const ImageInfo * EncoderObjectBrew::GetInfo( File *file ) /*= 0*/
{
...
bool EncoderObjectBrew::GetSurface( char8 *srcBuffer, int32 bufferSize, GraphicsSystem::Surface *destSurface, ImageInfo *pDestProps /*= NULL*/ ) /*= 0*/
{
	if (state != EES_IDLE)
	{
		return false;
...
bool EncoderObjectBrew::GetSurface( File *srcFile, GraphicsSystem::Surface *destSurface, ImageInfo *pDestProps /*= NULL*/ ) /*= 0*/
{
	if (state != EES_IDLE)
	{
		return false;
...
bool EncoderObjectBrew::GetData( GraphicsSystem::Surface *sourceSurface, char8 *destBuffer, int32 bufferSize ) /*= 0*/
{
...
void EncoderObjectBrew::LoadData(GraphicsSystem::Surface *destSurface, ImageInfo *img)
{
...
void EncoderObjectBrew::DecompressLines()
{
	GraphicsSystem *gr = GetApplication()->GetGraphicsSystem();
...
void EncoderObjectBrew::SetSourceToBuffer( char8 *buffer, int32 size )
{
	isOk = true;
...
void EncoderObjectBrew::SetSourceToFile( File *file )
{
	isOk = true;
...
bool EncoderObjectBrew::IsFree()
{
	return state == EES_IDLE;
}

bool EncoderObjectBrew::Resize( File *srcFile, char8 *destBuffer, int32 bufferSize, ImageInfo *pDestProps )
{
	if (state != EES_IDLE)
	{
		return false;
...
void EncoderObjectBrew::ResizeLines()
{
