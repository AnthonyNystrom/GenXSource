...
CAafImageHandler::CAafImageHandler(CFbsBitmap& aBitmap, RFs& aFs, MRequestObserver& aCallback)
	:	iBitmap(aBitmap), iFs(aFs), iCallback(aCallback), CActive(CActive::EPriorityStandard)
	{}

void CAafImageHandler::ConstructL()
	{
...
CAafImageHandler::~CAafImageHandler()
	{
	delete iLoadUtil;
	delete iSaveUtil;
...
void CAafImageHandler::LoadFileL(const TFileName& aFileName, TInt aSelectedFrame)
	{
...
void CAafImageHandler::SaveImageToFileL(const TFileName& aFileName, const TUid& aImageType, const TUid& aImageSubType)
{
	__ASSERT_ALWAYS(!IsActive(),User::Invariant());
...
void CAafImageHandler::LoadBufferL(TDesC8 &aSourceData, TInt aSelectedFrame /* = 0 */)
{
	__ASSERT_ALWAYS(!IsActive(),User::Invariant());
	// create a CImageDecoder to read the specified file
	delete iLoadUtil;
...
void CAafImageHandler::SaveImageToBufferL(HBufC8*& aDestinationData, const TDesC8 &aMIMEType)
	{
	__ASSERT_ALWAYS(!IsActive(), User::Invariant());
...
void CAafImageHandler::FrameRotate(TBool aClockwise)
	{
	__ASSERT_ALWAYS(!IsActive(),User::Invariant());
	// start rotating the bitmap: RunL called when complete
	if (aClockwise)
...
void CAafImageHandler::ZoomFrame(TBool aZoomIn)
	{
	__ASSERT_ALWAYS(!IsActive(),User::Invariant());
...
void CAafImageHandler::FitToThresholds()
	{
...
TBool CAafImageHandler::ExceedsThresholds()
{
	TSize size(iBitmap.SizeInPixels());
