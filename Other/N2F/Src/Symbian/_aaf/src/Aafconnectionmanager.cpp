...
CAafConnectionManager* CAafConnectionManager::iSelfInstance = NULL;

CAafConnectionManager::CAafConnectionManager()
{
	iSocketServ = NULL;
	iConnection = NULL;
	iIAPId = 0;
...
void CAafConnectionManager::ConstructL()
{
	...
	// Read IAP description from file
	ReadSavedIAPL();
}

CAafConnectionManager* CAafConnectionManager::GetInstanceL()
{
	if (!iSelfInstance)
	{
		iSelfInstance = new (ELeave)CAafConnectionManager;
...
void CAafConnectionManager::SelectIAPL()
{
	__LOGSTR_TOFILE("CAafConnectionManager::SelectIAPL() begins");
	...
	__LOGSTR_TOFILE("CAafConnectionManager::SelectIAPL() ends");
}

TInt CAafConnectionManager::InstallConnectionL()
{
	TInt retValue = 0;
...
TBool CAafConnectionManager::ReadSavedIAPL()
{
	__LOGSTR_TOFILE("CAafConnectionManager::ReadSavedIAPL() begins");
	...
	if (err != KErrNone)
	{
		...
		__LOGSTR_TOFILE("CAafConnectionManager::ReadSavedIAPL() failed to open");
	}
	...
	__LOGSTR_TOFILE("CAafConnectionManager::ReadSavedIAPL() ends");

	return retValue;
}

TBool CAafConnectionManager::SaveIAPL()
{
	__LOGSTR_TOFILE("CAafConnectionManager::SaveIAPL() begins");
	...
	if (iIAPId != 0)
	{
		...
		if (err != KErrNone)
		{
			...
			__LOGSTR_TOFILE("CAafConnectionManager::SaveIAPL() failed to open file");
		}

		if (retValue)
		{
			__LOGSTR_TOFILE("CAafConnectionManager::SaveIAPL() succeed to open the file");
	...
	__LOGSTR_TOFILE("CAafConnectionManager::SaveIAPL() ends");
