...
class com.jeroenwijering.players.FLVModel extends AbstractModel {
	...
	private function setStart(pos:Number) {
		...
		if(flvType=="RTMP" && feeder.feed[currentItem]["id"] != currentURL && _root.live=='true') {
			currentURL = feeder.feed[currentItem]["id"];
			connectObject.connect(feeder.feed[currentItem]["file"],"");
			
			trace('Connecting live to ' + currentURL)
			setStreamObject(connectObject);
			streamObject.play(currentURL);
		
		} else if (flvType=="RTMP" && feeder.feed[currentItem]["id"] != currentURL) {
			connectObject.connect(feeder.feed[currentItem]["file"]);
			currentURL = feeder.feed[currentItem]["id"];
			setStreamObject(connectObject);
			streamObject.play(feeder.feed[currentItem]["file"]);
			
			
			trace('CURRENT URL' + feeder.feed[currentItem]["file"]);
