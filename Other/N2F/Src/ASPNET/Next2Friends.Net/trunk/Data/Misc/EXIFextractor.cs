...
namespace Goheer
{
	...
	{	
		...
		public class EXIFextractor : IEnumerable
		{
            ...
            /// <summary>
            /// Gets the dateTime that the photo was taken
            /// </summary>
            public DateTime DTTaken 
            {
                get 
                {
                    try
                    {
                        string datetime = this["Date Time"].ToString();
                        string finaldt = datetime.Substring(0, datetime.Length - 1);
                        IFormatProvider culture = new CultureInfo("en-US", true);
                        DateTime dt = DateTime.ParseExact(finaldt, "yyyy:MM:dd HH:mm:ss", culture);
                        return dt;
                    }
                    catch
                    {
