...
class SettingsScreen extends AbstractWindow {
	...
	SettingsScreen(String id, Resources resoursable) {
		...
		cmdSubmit = new Command(resoursable.get("continueCommand"), Command.SCREEN, 1);
		edLogin = new TextField(resoursable.get("login"), bean.getLogin(), 32, TextField.ANY);
		edPassword = new TextField(resoursable.get("pass"), bean.getPassword(), 32, TextField.ANY | TextField.PASSWORD);
		...
		cmdBack = new Command(exit? resoursable.get("exitCommand"): resoursable.get("backCommand"), exit?Command.EXIT: Command.CANCEL, 1);
	...
	protected void refresh() {
		hideBusy();
		bean = (SettingsBean) UIManager.getInstance().getBean(UIManager.SCREEN_SETTINGS);
	...
	public void commandAction(Command c, Displayable d) {
		...
		if (c == cmdBack) {
			...
			if (!isValid(login = edLogin.getString(), pass = edPassword.getString())) {
				showAlert(getResoursable().get("alertCredentials"), AlertType.CONFIRMATION, this);
			} else {
				bean.setLogin(login);
