...
class CartScreen extends AbstractWindow {
	protected List form;
	protected Command backCommand, nextCommand, browseCommand;
	private Command previewCommand, deleteCommand, deleteAllCommand, selectCommand;
	
	protected final Vector items = new Vector();
	private final Image IMAGE_SEL = Engine.getEngine().getImage("/check.png");
	private final Image IMAGE_DESEL = Engine.getEngine().getImage("/file.png");;

	protected void setTitle() {
		this.form = new List(getLocalizedText(LocaleUI.FORM_IMAGE_CART_CAPTION), List.IMPLICIT);
		this.form.setTicker(new Ticker(getLocalizedText(LocaleUI.IMAGE_CART_TEXT)));
	}
	
	protected void createNextCommand(CartBean bean) {
		this.nextCommand = new Command(bean.nextIsProceedCommandName()? getLocalizedText(LocaleUI.IMAGE_CART_COMMAND_NEXT):getLocalizedText(LocaleUI.IMAGE_CART_COMMAND_ASK), Command.SCREEN, 1);
		this.form.addCommand(nextCommand);
	}

	protected CartBean getCardBean() {
	...
	CartScreen(String id, Resoursable resoursable) {
		...
		CartBean bean = getCardBean();
		setTitle();
		createNextCommand(bean);		
		this.backCommand = new Command(getLocalizedText(LocaleUI.CARD_COMMAND_BACK), Command.BACK, 9);
...
//			this.form.append(getLocalizedText(LocaleUI.IMAGE_CART_TEXT), null);			
	...
	private void previewItem() {
		...
		if (selectedIndex != -1) {
			ItemLight2 cItem = getCardBean().getItem(selectedIndex);
			bean.setItem(cItem);
...
//		}
	...
	public void commandAction(Command c, Displayable d) {
		...
		if (c == backCommand) {
			...
			if (getResoursable().isTouch()) {
				new CardOptionsScreen(getResoursable()).displayOptions();
			} else {
				markItem();				
			}
		}
