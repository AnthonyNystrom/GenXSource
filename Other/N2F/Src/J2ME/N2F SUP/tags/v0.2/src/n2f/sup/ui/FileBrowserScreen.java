...
import n2f.sup.common.file.FileInfoWrapper;
import n2f.sup.core.ActionItem;
import n2f.sup.core.Engine;
import n2f.sup.core.ItemLight2;
import n2f.sup.ui.bean.CartBean;
import n2f.sup.ui.bean.FileSelectorBean;
import n2f.sup.ui.bean.PreviewBean;


class FileBrowserScreen extends AbstractWindow {
	...
	private FileSelectorBean getFileSelectorBean() {
		FileSelectorBean bean = (FileSelectorBean)Engine.getEngine().getBean(UIManager.SCREEN_FILE_SYSTEM);
		if (bean == null) {
			Engine.getEngine().putBean(UIManager.SCREEN_FILE_SYSTEM, bean = new FileSelectorBean());
		}
		return bean;
	}
	
	public FileBrowserScreen(String id, Resoursable resoursable) {
	...
	protected String getTextAskCommand() {
		return getLocalizedText(LocaleUI.FILE_SELECTOR_COMMAND_GOTOCART);
	}
	
	protected String getTextCancel() {
		return getLocalizedText(LocaleUI.FILE_SELECTOR_COMMAND_CANCEL);
	}
	
	protected void createCommands() {
		this.askCommand = new Command(getTextAskCommand(), Command.SCREEN, 1);
		this.cancelCommand = new Command(getTextCancel(), Command.CANCEL, 9);
...
//		this.list.addCommand(cmdPrivateQuestions);
	...
	protected void add2Card(ItemLight2 item){
		if (item != null){ 
			if (item.getType() == ItemLight2.TYPE_FILE) {
		...
		if (c == cancelCommand) {
			...
			if (item != null) {
				if (item.getType() == ItemLight2.TYPE_FOLDER) {
					...
					if (getResoursable().isTouch()) {
						new OptionsScreen(getResoursable()).displayOptions();
					} else {
