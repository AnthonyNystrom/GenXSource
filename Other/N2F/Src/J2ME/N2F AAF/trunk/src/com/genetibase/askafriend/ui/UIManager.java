...
import com.genetibase.askafriend.utils.Console;
...
public class UIManager extends AbstractErrorManager implements Deallocatable, FileSystemListener, ErrorListener {
    ...
    public static final String SCREEN_SAVE = LocaleUI.FORM_SAVE_CAPTION;
	public static final String SCREEN_CONTACTS = LocaleUI.FORM_CONTACTS_CAPTION;
	public static final String SCREEN_SMS = LocaleUI.FORM_SMS_PREVIEW_CAPTION;
	public static final String SCREEN_SMS_EDIT = LocaleUI.FORM_SMS_EDIT_CAPTION;
	public static final String SCREEN_CAN_QUESTIONS = LocaleUI.FORM_CAN_QUESTIONS;

    private String defaultID = SCREEN_START;
    private boolean hasNewResponces;
...
//			add2stack = false;
		...
		} else if (id == SCREEN_CAN_QUESTIONS){
			currentUI = new CannedQuestionsScreen(id, getResoursable());
		} else {
...
//		}
	...
	public void rootChanged(int event, String root) 
	{
		Engine.getEngine().openInit(getCurrentID()==SCREEN_FILE_SYSTEM ||
				getCurrentID() == SCREEN_IMAGE_SAVER? getCurrentUI(): null);
	}
...
//				}
				if (getCurrentID() == SCREEN_START) {
					text = getLocalizedText(LocaleUI.REPLY_READY);
					getCurrentUI().showAlert(text, AbstractWindow.COMMAND_REPLIES, AbstractWindow.COMMAND_CANCEL, getCurrentUI());
				} else {
					hasNewResponces = true;
				}
...
//				}
				...
				case ErrorEvent.GET_CREDENTIALS:
					...
					break;
				case ErrorEvent.GET_PIM_CONTACTS:
					text = getLocalizedText(LocaleUI.ERROR_PIM_TEXT);
					break;	
				case ErrorEvent.SEND_SMS:
					text = getLocalizedText(LocaleUI.ERROR_SMS_TEXT);
					break;	
				default:
...
//			System.out.println("KEY=" + key);
	...
	public boolean isHasNewQuestions() {
		return hasNewQuestions;
	}

	public void setHasNewQuestions(boolean hasNewQuestions) {
		this.hasNewQuestions = hasNewQuestions;
	}

	public boolean isHasNewResponces() {
		return hasNewResponces;
	}

	public void setHasNewResponces(boolean hasNewResponces) {
		this.hasNewResponces = hasNewResponces;
	}
}
