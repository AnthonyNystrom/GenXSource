...
void VertScene( float4 iPos : POSITION,
                ...
                out float4 vPosLight : TEXCOORD2,
...
float4 PixScene( //float2 Tex : TEXCOORD0,
                 ...
                 float4 vPosLight : TEXCOORD2,
                 float4 clr : COLOR) : COLOR
{
    float4 Diffuse;
    ...
    if(true)// dot( vLight, g_vLightDir ) > g_fCosTheta ) // Light must face the pixel (within Theta)
    {
        ...
        float LightAmount = lerp( lerp( sourcevals[0], sourcevals[1], lerps.x ),
                                  ...
                                  lerps.y );
        // Light it
        Diffuse = saturate( dot( -vLight, normalize( vNormal ) ) ) * LightAmount * clr;// * ( 1 - g_vLightAmbient ) + g_vLightAmbient )
    } else
    {
        Diffuse = /*g_vLightAmbient **/ clr;
    }
