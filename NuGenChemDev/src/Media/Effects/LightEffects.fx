...
voPT0C0 VS_STD_TextureNoSpec(viPNC0T0 IN) 
{
    ...
    float4 ambColor = materialDiffuse * lightAmbient;
    float4 diffColor = materialDiffuse * diff * lightColor ;*/
    OUT.diffAmbColor = float4(1,1,1,1);//diffColor + ambColor;
...
voPC0 VS_STD_NoTextureNoSpecLine(viPC0 IN) 
{
    voPC0 OUT;
    OUT.hPosition = mul(float4(IN.position.xyz , 1.0), worldViewProj);

    float4 ambColor = IN.diffColor;
    float4 diffColor =  IN.diffColor;
    OUT.diffAmbColor = diffColor + ambColor;
...
technique std_basicDirLightNoSpecTexture
{
	pass p0 
    {
		VertexShader = compile vs_2_0 VS_STD_TextureNoSpec();
		PixelShader  = compile ps_2_0 PS_TextureNoSpec();
    }
}
