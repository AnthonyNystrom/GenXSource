...
namespace Genetibase.NuGenVisiCalc
{
	...
	internal sealed class Canvas : Panel
	{
		...
		public Color SchemaBackColor
		{
			...
			set
			{
				if (_pictureBox.BackColor != value)
				{
		...
		private NuGenTabPage _parentTabPage;

		internal NuGenTabPage ParentTabPage
		{
			get
			{
				return _parentTabPage;
			}
			set
			{
				_parentTabPage = value;
			}
		}
		...
		internal String Path
		{
			...
			set
			{
				if (_path != value)
				{
					...
					{
						ParentTabPage.Text = System.IO.Path.GetFileName(_path);
					}
				}
		...
		private Boolean _saveIsNeeded;

		internal Boolean SaveIsNeeded
		{
			get
			{
				return _saveIsNeeded;
			}
			set
			{
				if (_saveIsNeeded != value)
				{
					_saveIsNeeded = value;
					OnSaveIsNeededChanged(EventArgs.Empty);
				}
			}
		}

		internal event EventHandler SaveIsNeededChanged;

		private void OnSaveIsNeededChanged(EventArgs e)
		{
			if (SaveIsNeededChanged != null)
			{
				SaveIsNeededChanged(this, e);
			}
		}
