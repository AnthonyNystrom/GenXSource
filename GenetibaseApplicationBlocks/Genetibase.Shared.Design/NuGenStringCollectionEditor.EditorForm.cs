...
namespace Genetibase.Shared.Design
{
	partial class NuGenStringCollectionEditor
	{
		private sealed class EditorForm : CollectionForm
		{
			...
			private NuGenStringCollectionEditor _editor;
			...
			protected override void OnEditValueChanged()
			{
				...
				string editBoxText = string.Empty;

				for (int i = 0; i < items.Length; i++)
				{
					if (items[i] is string)
					{
						editBoxText = editBoxText + ((string)items[i]);

						if (i != (items.Length - 1))
						{
							editBoxText = editBoxText + System.Environment.NewLine;
						}
					}
				}

				_editBox.Text = editBoxText;
