...
namespace Genetibase.Shared.Controls.ControlReflectorInternals
{
	...
	public sealed class NuGenReflectedImageGenerator : INuGenReflectedImageGenerator
	{
		...
		public void BuildReflectedImage(Bitmap ctrlBmp, NuGenReflectStyle reflectStyle, int opacity)
		{
			...
			this.RotateBitmap(ctrlBmp, reflectStyle);
				...
				switch (reflectStyle)
				{
					...
					case NuGenReflectStyle.Right:
					{
						for (int y = 0; y < ctrlBmpData.Height; y++)
						{
							...
							{
								uint currentOpacity = this.GetCurrentOpacity(ctrlBmpSize, x, reflectStyle, opacity);
								this.ApplyOpacity(scan0, scanWidth, x, y, currentOpacity);
							}
						}
					...
					default:
					{
						for (int y = 0; y < ctrlBmpData.Height; y++)
						{
							uint currentOpacity = this.GetCurrentOpacity(ctrlBmpSize, y, reflectStyle, opacity);
		...
		public Bitmap GetControlImage(Control ctrl)
		{
			...
			Rectangle rect = this.GetImageRectangle(ctrl);
		...
		private unsafe void ApplyOpacity(uint* scan0, int scanWidth, int x, int y, uint currentOpacity)
		{
		...
		private uint GetCurrentOpacity(Size bmpSize, int iteration, NuGenReflectStyle reflectStyle, int maxOpacity)
		{
			return (uint)Math.Min(
				255,
				Math.Max(
					0,
					Math.Round(this.GetRawOpacity(bmpSize, iteration, reflectStyle, maxOpacity))
				)
			);
		}
		...
		private Rectangle GetImageRectangle(Control ctrl)
		{
		...
		private float GetRawOpacity(Size bmpSize, int iteration, NuGenReflectStyle reflectStyle, int maxOpacity)
		{
			switch (reflectStyle)
			{
				case NuGenReflectStyle.Left:
		...
		private void RotateBitmap(Bitmap bmpToRotate, NuGenReflectStyle reflectStyle)
		{
