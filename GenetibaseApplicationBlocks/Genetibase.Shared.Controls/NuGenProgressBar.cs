...
namespace Genetibase.Shared.Controls
{
	...
	[Designer(typeof(NuGenOrientationControlDesigner))]
	...
	public partial class NuGenProgressBar : NuGenOrientationControlBase
	{
		...
		protected virtual void OnMarqueeTimerTick(EventArgs e)
		{
			...
			this.Initiator.InvokeAction(_marqueeTimerTick, e);

			Debug.Assert(this.MarqueeBlockData != null, "this.MarqueeBlockData != null");
			Rectangle marqueeBounds = this.MarqueeBlockData.GetBounds(this.ClientRectangle, this.Orientation);

			if (this.Orientation == NuGenOrientationStyle.Horizontal)
			{
				this.MarqueeBlockData.Offset += 10;

				if (this.MarqueeBlockData.Offset > this.ClientRectangle.Right)
				{
					this.ResetOffset(NuGenOrientationStyle.Horizontal);
				}
			}
			else
			{
				...
				if (this.MarqueeBlockData.Offset < this.ClientRectangle.Top - marqueeBounds.Height)
				{
					this.ResetOffset(NuGenOrientationStyle.Vertical);
				}
			}
		...
		private NuGenPositiveInt32 _maximum = null;

		/// <summary>
		/// </summary>
		protected NuGenPositiveInt32 MaximumInternal
		{
			get
			{
				if (_maximum == null)
				{
					_maximum = new NuGenPositiveInt32();
					_maximum.Value = this.DefaultMaximum;
				}
		...
		{
			Debug.Assert(this.MaximumInternal != null, "this.MaximumInternal != null");
			this.MaximumInternal.Value = this.DefaultMaximum;
		}

		private bool ShouldSerializeMaximum()
		{
			Debug.Assert(this.MaximumInternal != null, "this.MaximumInternal != null");
			return this.MaximumInternal.Value != this.DefaultMaximum;
		}
		...
		private NuGenPositiveInt32 _minimum = null;

		/// <summary>
		/// </summary>
		protected NuGenPositiveInt32 MinimumInternal
		{
			get
			{
				if (_minimum == null)
				{
					_minimum = new NuGenPositiveInt32();
					_minimum.Value = this.DefaultMinimum;
				}
		...
		{
			Debug.Assert(this.MinimumInternal != null, "this.MinimumInternal != null");
			this.MinimumInternal.Value = this.DefaultMinimum;
		}

		private bool ShouldSerializeMinimum()
		{
			Debug.Assert(this.MinimumInternal != null, "this.MinimumInternal != null");
			return this.MinimumInternal.Value != this.DefaultMinimum;
		}
		...
		public NuGenProgressBarStyle Style
		{
			...
			set
			{
				if (_style != value)
				{
					...
					{
						Debug.Assert(this.MarqueeBlockData != null, "this.MarqueeBlockData != null");
						this.ResetOffset(this.Orientation);
		...
		private MarqueeBlock _marqueeBlockData = null;

		/// <summary>
		/// </summary>
		protected virtual MarqueeBlock MarqueeBlockData
		{
			get
			{
				if (_marqueeBlockData == null)
				{
					_marqueeBlockData = new MarqueeBlock();
				}

				return _marqueeBlockData;
			}
		}
		...
		protected override void OnPaintBackground(PaintEventArgs e)
		{
			...
			NuGenProgressBarPaintParams paintParams = new NuGenProgressBarPaintParams(
				this,
				e.Graphics,
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			switch (this.Style)
			{
				case NuGenProgressBarStyle.Marquee:
				{
					...
					{
						foreBounds = this.GetMarqueeRectangle(this.ClientRectangle);
					}
					break;
				}
				default:
				{
					foreBounds = this.GetContinuousRectangle(this.ClientRectangle);
					break;
				}
			}
			...
			this.Renderer.DrawForeground(new NuGenProgressBarPaintParams(this, g, foreBounds, state, this.Style));
			this.Renderer.DrawBorder(new NuGenPaintParams(this, g, bounds, state));
		}
		...
		/*
		 * GetContinuousRectangle
		 */

		private Rectangle GetContinuousRectangle(Rectangle bounds)
		{
			Rectangle valueRectangle = Rectangle.Empty;
			...
			if (this.Maximum != this.Minimum)
			{
				if (this.Orientation == NuGenOrientationStyle.Horizontal)
				{
					valueRectangle = new Rectangle(
						bounds.Left,
						bounds.Top,
						(int)((float)bounds.Width / (float)(this.Maximum - this.Minimum) * (this.Value - this.Minimum)),
						bounds.Height
					);
				}
				else
				{
					int height = (int)((float)bounds.Height / (float)(this.Maximum - this.Minimum) * (this.Value - this.Minimum));
		...
		private Rectangle GetMarqueeRectangle(Rectangle bounds)
		{
			return this.MarqueeBlockData.GetBounds(bounds, this.Orientation);
		}
