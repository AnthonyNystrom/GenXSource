...
namespace Genetibase.Shared.Windows
{
	...
	public class NuGenWndLessControl : NuGenEventInitiator
	{
		...
		protected virtual void OnEnabledChanged(EventArgs e)
		{
			...
			this.InvokePropertyChanged(_enabledChanged, e);
		}
		...
		{
			this.InvokePropertyChanged(_parentChanged, e);
		}
		...
		private void SubscribeParentEvents(Control parent)
		{
			if (parent != null)
			{
				parent.Click += this.Parent_Click;
		...
		private bool _mouseButtonPressedInside = false;
		...
		private void Parent_MouseDown(object sender, MouseEventArgs e)
		{
			if (this.Enabled && this.Bounds.Contains(e.Location))
			{
				...
				this.OnMouseDown(e);
				this.InvokeMouseAction(_mouseDown, e);
			}
		}
		...
		private void Parent_MouseEnter(object sender, EventArgs e)
		{
			...
			{
				this.OnMouseEnter(e);
				this.InvokeAction(_mouseEnter, e);
			}
		}
		...
		private void Parent_MouseLeave(object sender, EventArgs e)
		{
			...
			{
				this.OnMouseLeave(e);
				this.InvokeAction(_mouseLeave, e);
			}
		}
			...
			if (this.Enabled)
			{
				if (this.Bounds.Contains(e.Location))
				{
					if (!_mouseInside)
					{
						...
						this.InvokeAction(_mouseEnter, EventArgs.Empty);

						_mouseInside = true;
					}

					this.OnMouseMove(e);
					this.InvokeMouseAction(_mouseMove, e);
				}
				else if (!this.Parent.Capture)
				{
					if (_mouseInside)
					{
						...
						this.InvokeAction(_mouseLeave, EventArgs.Empty);
		...
		private void Parent_MouseUp(object sender, MouseEventArgs e)
		{
			if (_mouseButtonPressedInside)
			{
				...
				this.OnMouseUp(e);
				this.InvokeMouseAction(_mouseUp, e);
			}
		}
