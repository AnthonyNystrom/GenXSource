...
namespace Genetibase.Shared.Drawing
{
	partial class NuGenControlPaint
	{
		...
		/*
		 * TextBoundsFromImageBounds
		 */

		/// <summary>
		/// Use <see cref="StringFormat"/> to position the text within the retrieved bounds.
		/// </summary>
		public static Rectangle TextBoundsFromImageBounds(Rectangle bounds, Rectangle imageBounds, ContentAlignment imageAlign)
		{
			Rectangle textBounds = Rectangle.FromLTRB(imageBounds.Right, bounds.Top, bounds.Right, bounds.Bottom);

			if ((imageAlign & NuGenControlPaint.AnyCenter) != 0)
			{
				textBounds = bounds;
			}
			else if ((imageAlign & NuGenControlPaint.AnyRight) != 0)
			{
				textBounds = Rectangle.FromLTRB(
					bounds.Left,
					bounds.Top,
					imageBounds.Left,
					bounds.Bottom
		...
		{
			if ((alignmentToConvert & NuGenControlPaint.AnyRight) != 0)
			{
				return StringAlignment.Far;
			}

			if ((alignmentToConvert & NuGenControlPaint.AnyCenter) != 0)
			{
