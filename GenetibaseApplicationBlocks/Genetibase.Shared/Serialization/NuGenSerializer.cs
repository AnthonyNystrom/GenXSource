...
namespace Genetibase.Shared.Serialization
{
	...
	public class NuGenSerializer : NuGenEventInitiator
	{
		...
		private INuGenObjectStringConverter _converter = null;
		private NuGenGraph _graph = null;
		private NuGenReferenceCollection _references = null;
		...
		private bool _checkSerializable = false;
		...
		/// </exception>
		public object GetDefaultValue(MemberDescriptor member)
		{
			if (member == null)
			{
		...
		/// <paramref name="obj"/> is <see langword="null"/>.
		/// </exception>
		public void SetProperty(PropertyInfo propertyInfo, object obj, object value)
		{
		...
		public void SetReferenceDeserializing()
		{
			...
			foreach (NuGenReference reference in _references)
			{
				...
				if (obj != null)
				{
					this.SetProperty(reference.StandardPropertyInfo, reference.Object, obj);
				}
			...
			while (!xmlTextReader.EOF)
			{
				if (xmlTextReader.IsStartElement())
				{
					...
					else if (xmlTextReader.GetAttribute(Resources.XmlAttribute_IsSer) != null)
					{
						...
						{
							propertyInfo.ReferenceCode = int.Parse(refAttrValue);
						}
					...
					{
						propertyInfo.ReferenceCode = int.Parse(xmlTextReader.GetAttribute(Resources.XmlAttribute_IsRef));
					}
					else if (propertyInfo.IsList)
					{
						propertyInfo.Count = int.Parse(xmlTextReader.GetAttribute(Resources.XmlAttribute_Count));
					...
					else if (!propertyInfo.IsKey)
					{
						...
						{
							int bufferLength = int.Parse(xmlTextReader.GetAttribute(Resources.XmlAttribute_Length));
					...
					else if (propertyInfo.IsKey)
					{
						...
						if (attrRef != null)
						{
							propertyInfo.ReferenceCode = int.Parse(attrRef);
						}
		...
		private void DeserializeObject(object obj, NuGenPropertyInfoCollection properties)
		{
			foreach (NuGenPropertyInfo property in properties)
			{
				...
				if (property.IsKey)
				{
					this.SetProperty(propertyInfo, obj, property.Value);
				...
				else
				{
						...
						if (propertyInfo != null)
						{
							...
							if (list != null)
							{
								...
								if (list is Array)
								{
									...
									this.SetProperty(propertyInfo, obj, list);
								}
								else
								{
									list.Clear();
					...
					if (property.IsReference)
					{
						...
						this.SetProperty(propertyInfo, obj, propertyValue);
					...
					else
					{
						...
						{
							this.SetProperty(propertyInfo, obj, null);
							continue;
						}
					...
					if (index == -1)
					{
						...
						if (this.CheckSerializable && (item is INuGenSerializable))
						{
							propertyInfo = new NuGenPropertyInfo(
								string.Format("{0}{1}", Resources.XmlTag_Item, (count + 1).ToString()),
								item,
								true,
								false,
								false
						...
						else
						{
							...
							propertyInfo = new NuGenPropertyInfo(
								string.Format("{0}{1}", propertyName, (count + 1).ToString()),
								item,
								true,
								false,
								false
		...
		public NuGenPropertyInfoCollection SerializeObject(object obj)
		{
			...
			for (int i = 0; i < descriptors.Count; i++)
			{
				...
				object propertyDefaultValue = this.GetDefaultValue(descriptor);
						...
						case NuGenSerializationVisibility.Class:
						{
							if (!propertyValue.GetType().IsClass)
							{
								throw new InvalidCastException(
									string.Format("{0} is not a class.", propertyName)
									);
							}
		...
		private void SerializeProperty(XmlTextWriter xmlTextWriter, NuGenPropertyInfo propertyInfo)
		{
			...
			else if (propertyInfo.IsKey)
			{
				...
				{
					xmlTextWriter.WriteStartElement(propertyName);
					xmlTextWriter.WriteAttributeString(Resources.XmlAttribute_IsRef, propertyInfo.ReferenceCode.ToString());
					xmlTextWriter.WriteEndElement();
				}
				else
				{
					...
					{
						xmlTextWriter.WriteAttributeString(Resources.XmlAttribute_Ref, propertyInfo.ReferenceCode.ToString());
					}
			...
			else if (propertyInfo.IsList)
			{
				...
				if (propertyInfo.Value != null)
				{
					xmlTextWriter.WriteAttributeString(Resources.XmlAttribute_Count, propertyInfo.Count.ToString());
				}
			...
			else if (propertyInfo.Value is Metafile)
			{
				...
				xmlTextWriter.WriteAttributeString(Resources.XmlAttribute_Length, buffer.Length.ToString());
