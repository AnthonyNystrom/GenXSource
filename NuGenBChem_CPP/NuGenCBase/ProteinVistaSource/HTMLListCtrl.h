...
//Events
#define HTMLLIST_SELECTED			1	
#define HTMLLIST_DESELECTED			2
#define HTMLLIST_LBUTTONDOWN		3
#define HTMLLIST_RBUTTONDOWN		4
#define HTMLLIST_LBUTTONDBLCLICK	5
#define HTMLLIST_ITEMCHECKED		6
...
private:
	...
	COLORREF	 m_clrTextSelectedItem;
	COLORREF	 m_clrBkCtrlItem;
	COLORREF	 m_clrTextCtrlItem;
...
public:
	...
	void SendSelectionChangeNotification(int nPosOld, int nPos);
	...
	int GetSelectedItem();
	void SetSelectedItem(long pos) { m_nSelectedItem = pos; }	//	select와 deselect 메시지를 모두 처리해야 한다.
	//	void DeselectAllItem(){ SendSelectionChangeNotification(m_nSelectedItem, -1); m_nSelectedItem = NONE_SELECTED; Invalidate(TRUE); }
