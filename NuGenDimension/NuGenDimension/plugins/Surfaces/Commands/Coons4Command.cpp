...
#include "Coons4Command.h"

#include "..//resource.h"

Coons4Command::Coons4Command(IApplicationInterface* appI):
						m_app(appI)
{
	ASSERT(m_app);
	m_step = 0;
	...
	m_get_object_panels[0] = 
		...
		m_get_object_panels[2] = 
		m_get_object_panels[3] = NULL;
...
bool    Coons4Command::PreTranslateMessage(MSG* pMsg)
{
	try {
...
void     Coons4Command::SendCommanderMessage(ICommander::COMMANDER_MESSAGE mes, 
										   void* params) 
{
		...
		SWITCH_RESOURCE
			switch(m_step) 
			{
				...
				case 2:
					...
					break;
				case 3:
					m_message.LoadString(IDS_COON_SEL_4_CONT);
					break;
				default:
					ASSERT(0);
					break;
...
void  Coons4Command::Start()	
{
	...
	m_get_object_panels[3] = reinterpret_cast<IGetObjectsPanel*>(m_app->
			GetCommandPanel()->
			AddDialog(IBaseInterfaceOfGetDialogs::GET_OBJECTS_DLG,m_message,true));
...
void  Coons4Command::MouseMove(unsigned int nFlags,int pX,int pY)
{
	//if (m_step==0)
	{
...
bool  Coons4Command::CheckObjects()
{
	if (m_objs[0]==NULL || m_objs[1]==NULL || m_objs[2]==NULL || m_objs[3]==NULL)
		return false;
...
void  Coons4Command::LeftClick(unsigned int nFlags,int pX,int pY)
{
	SWITCH_RESOURCE
		...
		if (m_cur_obj)
		{
			...
			if (m_step==4)
			{
				if (!CheckObjects())
				{
					...
					m_app->PutMessage(IApplicationInterface::MT_ERROR,m_message);
					m_objs[3]->Select(false);
					m_objs[3] = NULL;
					return;
				}
				sgCObject* aaa = sgSurfaces::Coons((sgC2DObject&)(*m_objs[0]),
					(sgC2DObject&)(*m_objs[1]),
					(sgC2DObject&)(*m_objs[2]),
					(sgC2DObject*)(m_objs[3]),36,36,6,6);
				if (!aaa)
				{
...
void  Coons4Command::Draw()
{
}


void  Coons4Command::OnEnter()
{
	SWITCH_RESOURCE
		m_cur_obj=m_app->GetViewPort()->GetHotObject();
	if (m_cur_obj->IsSelect())
...
unsigned int  Coons4Command::GetItemsCount()
{
	return 0;
}

void         Coons4Command::GetItem(unsigned int itemID, CString& itSrt)
{
	
}

void     Coons4Command::GetItemState(unsigned int itemID, bool& enbl, bool& checked)
{
	
}

HBITMAP   Coons4Command::GetItemBitmap(unsigned int)
{
	return NULL;
}

void         Coons4Command::Run(unsigned int itemID)
{
