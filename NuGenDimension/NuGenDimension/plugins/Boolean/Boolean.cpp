...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Boolean.DLL Initializing!\n");
	...
	{
		TRACE0("Boolean.DLL Terminating!\n");
...
extern "C" AFX_EXT_API   void GetButtonState(unsigned int nID, bool& ch, bool& enbl)
{
	...
	if ((nID==startID) && (active_regime == REGIME_INTERSECTION))
	{
		ch=true;
	    return;
	}
	if ((nID==startID+1) && (active_regime == REGIME_UNION))
	{
		ch=true;
		return;
	}
	if ((nID==startID+2) && (active_regime == REGIME_SUB))
	{
		ch=true;
		return;
	}
	if ((nID==startID+3) && (active_regime == REGIME_INT_LINE))
	{
		ch=true;
		return;
	}
...
extern "C" AFX_EXT_API   ICommander* GetNewCommander(unsigned int nID,IApplicationInterface* appI)
{
	if (nID == startID)
		{
			active_regime = REGIME_INTERSECTION;
			global_current_commander = new BooleanCommand(appI,BO_INTSCT);
			return global_current_commander;
		}
		else if (nID == startID+1)
		{
			active_regime = REGIME_UNION;
			global_current_commander = new BooleanCommand(appI,BO_UNION);
			return global_current_commander;
		}
		else if (nID == startID+2)
		{
			active_regime = REGIME_SUB;
			global_current_commander = new BooleanCommand(appI,BO_SUB);
			return global_current_commander;
		}
		else if (nID == startID+3)
		{
			active_regime = REGIME_INT_LINE;
			global_current_commander = new BooleanCommand(appI,BO_IL);
			return global_current_commander;
		}
		else if (nID == startID+4)
		{
