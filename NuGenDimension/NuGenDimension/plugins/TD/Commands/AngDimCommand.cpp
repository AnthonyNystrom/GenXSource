...
#include "AngDimCommand.h"

#include "..//resource.h"

int     ang_dim_name_index = 1;

AngDimCommand::AngDimCommand(IApplicationInterface* appI):
						m_app(appI)
							,m_get_first_point_panel(NULL)
							,m_get_second_point_panel(NULL)
							,m_get_third_point_panel(NULL)
							,m_get_fourth_point_panel(NULL)
		...
		
{
	...
	m_dim_style.invert = false;
}

AngDimCommand::~AngDimCommand()
{	
...
void     AngDimCommand::SendCommanderMessage(ICommander::COMMANDER_MESSAGE mes, 
										  void* params) 
{
...
bool    AngDimCommand::PreTranslateMessage(MSG* pMsg)
{
	try { //#try
				...
				if (m_props_panel && m_props_panel->IsInFocus())
					...
					else
					{
						...
						case 2:
							...
							break;
						case 3:
							if (m_get_fourth_point_panel)
								m_get_fourth_point_panel->GetWindow()->SendMessage(pMsg->message,
								pMsg->wParam,
								pMsg->lParam);
							break;
						case 4:
...
void  AngDimCommand::MouseMove(unsigned int nFlags,int pX,int pY)
{
	...
	case 2:
		...
		break;
	case 3:
		this_step = m_get_fourth_point_panel;
		break;
	case 4:
...
void  AngDimCommand::LeftClick(unsigned int nFlags,int pX,int pY)
{
	SWITCH_RESOURCE
	if (m_step==0)
	{
		...
		m_app->GetCommandPanel()->EnableRadio(8,true);
		...
		if (m_step==1)
		{
			...
			if (m_step==2)
			{
				...
				if (m_step==3)
				{
					if (sgSpaceMath::PointsDistance(m_third_pnt,m_cur_pnt)<0.000001)
					{
						m_message.LoadString(IDS_ERROR_POINT_IS_EQ);
						m_app->PutMessage(IApplicationInterface::MT_ERROR,
							m_message);
					...
					if (m_step==4)
					{
						...
						sgCDimensions* dm = sgCDimensions::Create(SG_DT_ANGLE,ppp,
						...
						CString nmInd;
						nmInd.Format("%i",ang_dim_name_index);
						nm+=nmInd;
...
void  AngDimCommand::Draw()
{
	...
	case 4:
		...
		ppp[3] = m_fourth_pnt;
		ppp[4] = m_cur_pnt;
		sgCDimensions::Draw(SG_DT_ANGLE,ppp,
			sgFontManager::GetFont(sgFontManager::GetCurrentFont()),m_dim_style,
...
unsigned int  AngDimCommand::GetItemsCount()
{
	return 0;
}

void         AngDimCommand::GetItem(unsigned int itemID, CString& itSrt)
{
	
}

void     AngDimCommand::GetItemState(unsigned int itemID, bool& enbl, bool& checked)
{
	
}

HBITMAP   AngDimCommand::GetItemBitmap(unsigned int)
{
	return NULL;
}

void         AngDimCommand::Run(unsigned int itemID)
{
	
}

void     AngDimCommand::NewScenar()
{
	SWITCH_RESOURCE
	...
	m_props_panel = new CLinearDimDlg(m_app->GetCommandPanel(), &m_dim_style, m_app,true,
