...
CEGPropertyGridList::CEGPropertyGridList() :
	m_pProp(NULL),
...
BEGIN_MESSAGE_MAP(CEGPropertyGridList, CWnd)
	//{{AFX_MSG_MAP(CEGPropertyGridList)
	ON_WM_SIZE()
	ON_WM_PAINT()
	ON_WM_SETCURSOR()
	ON_WM_LBUTTONDOWN()
...
/////////////////////////////////////////////////////////////////////////////
// CEGPropertyGridList message handlers

void CEGPropertyGridList::SetPropOwner(CEGPropertyGrid* pProp)
{
	m_pProp = pProp;
}


BOOL CEGPropertyGridList::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
	CWnd* pWnd = this;
...
void CEGPropertyGridList::OnSize(UINT nType, int cx, int cy) 
{
...
void CEGPropertyGridList::RecreateBackBuffer(int cx, int cy)
{
...
void CEGPropertyGridList::UpdateResize()
{
	SCROLLINFO si;
	LONG nHeight;
	CRect rc;
...
void CEGPropertyGridList::OnPaint() 
{
	CPaintDC dc(this);
	CDC memdc;
	CBitmap* pOldBitmap;
...
BOOL CEGPropertyGridList::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
void CEGPropertyGridList::OnLButtonDown(UINT, CPoint point) 
{
	ASSERT(m_pProp!=NULL);
	...
	switch (nHit)
	{
		...
		default:
			if ((pItem = m_pProp->FindItem(point))!=NULL)
			{
				CEGPropertyGridItem* pOldFocus = m_pProp->GetFocusedItem();
...
void CEGPropertyGridList::OnLButtonUp(UINT, CPoint point) 
{
	if (m_bColDrag)
	{
		CDC* pDC = GetDC();
...
void CEGPropertyGridList::OnLButtonDblClk(UINT, CPoint point)
{
	ASSERT(m_pProp!=NULL);
...
void CEGPropertyGridList::OnMouseMove(UINT, CPoint point)
{
	if (m_bColDrag)
	{
		CDC* pDC = GetDC();
...
BOOL CEGPropertyGridList::OnMouseWheel(UINT, short zDelta, CPoint) 
{
	SCROLLINFO si;
...
void CEGPropertyGridList::OnKeyDown(UINT nChar, UINT, UINT) 
{
...
UINT CEGPropertyGridList::OnGetDlgCode() 
{
	return DLGC_WANTARROWS|DLGC_WANTCHARS|DLGC_WANTALLKEYS;
}


void CEGPropertyGridList::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar*) 
{
	SCROLLINFO si;
	CRect rc;
	LONG nHeight;
...
void CEGPropertyGridList::CheckVisibleFocus()
{
	ASSERT(m_pProp!=NULL);
