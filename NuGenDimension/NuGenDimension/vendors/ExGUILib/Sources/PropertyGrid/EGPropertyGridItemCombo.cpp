...
void CEGPropertyGridItemCombo::DrawAttribute(CDC* pDC, const RECT& rc)
{
	...
	if (!IsWindow(m_hWnd))
	{
		TRACE0("CEGPropertyGridItemCombo::DrawAttribute() - The window has not been created\n");
		return;
	}
...
BOOL CEGPropertyGridItemCombo::CreateComboBox(DWORD dwStyle)
{
	...
	{
		TRACE0("CEGPropertyGridItemCombo::CreateComboBox() - failed to create combo box\n");
		return FALSE;
	}
...
BOOL CEGPropertyGridItemCombo::CreateComboBoxBool()
{
	...
	if (!Create(dwStyle, CRect(0,0,0,0), m_pProp->GetCtrlParent(), GetCtrlID()))
	{
		TRACE0("CEGPropertyGridItemCombo::CreateComboBoxBool() - failed to create combo box\n");
		return FALSE;
	}
...
LONG CEGPropertyGridItemCombo::FindCBData(LPARAM lParam)
{
	LONG idx;
...
void CEGPropertyGridItemCombo::OnSelchange() 
{
	CommitChanges();
}


void CEGPropertyGridItemCombo::OnKillfocus() 
{
	CommitChanges();
}


void CEGPropertyGridItemCombo::SetDropDownHeight(LONG nDropHeight)
{
	m_nDropHeight = nDropHeight;
}


LONG CEGPropertyGridItemCombo::GetDropDownHeight()
{
