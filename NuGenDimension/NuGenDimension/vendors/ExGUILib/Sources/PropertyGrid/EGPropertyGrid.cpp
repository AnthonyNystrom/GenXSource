...
UINT CEGPropertyGrid::s_nInstanceCount;
CFont* CEGPropertyGrid::s_pNormalFont;
CFont* CEGPropertyGrid::s_pBoldFont;
CEGPropertyGridItem* CEGPropertyGrid::s_pFound;
...
BEGIN_MESSAGE_MAP(CEGPropertyGrid, CWnd)
	//{{AFX_MSG_MAP(CEGPropertyGrid)
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_WM_ENABLE()
...
/////////////////////////////////////////////////////////////////////////////
// CEGPropertyGrid message handlers

const POINT& CEGPropertyGrid::GetOrigin()
{
	return m_Origin;
}
...
int CEGPropertyGrid::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	{
		TRACE0("Failed to create CEGPropertyGridList\n");
		return -1;
	}

	m_List.SetPropOwner(this);

	// create CEGPropertyGridInfo
	//
	...
	{
		TRACE0("Failed to create CEGPropertyGridInfo\n");
		return -1;
	}
...
CWnd* CEGPropertyGrid::GetCtrlParent()
{
	return &m_List;
}
...
void CEGPropertyGrid::ResizeChildWindows(int cx, int cy)
{
	if (m_bShowInfo)
	{
...
void CEGPropertyGrid::InitGlobalResources()
{
...
void CEGPropertyGrid::FreeGlobalResources()
{
	if (s_pNormalFont)
	{
...
CFont* CEGPropertyGrid::GetNormalFont()
{
	return s_pNormalFont;
}


CFont* CEGPropertyGrid::GetBoldFont()
{
	return s_pBoldFont;
}


CEGPropertyGridItem* CEGPropertyGrid::GetFocusedItem()
{
	return m_pFocus;
}


CEGPropertyGridItem* CEGPropertyGrid::GetRootItem()
{
	return &m_Root;
}


void CEGPropertyGrid::ClearVisibleList()
{
	m_pVisbleList = NULL;
}


CEGPropertyGridItem* CEGPropertyGrid::GetVisibleList()
{
	return m_pVisbleList;
}


void CEGPropertyGrid::AddToVisibleList(CEGPropertyGridItem* pItem)
{
	if (!pItem)
		return;
...
BOOL CEGPropertyGrid::EnumItems(CEGPropertyGridItem* pItem, ENUMPROPITEMPROC proc, LPARAM lParam)
{
	if (!pItem || !proc)
		return FALSE;
...
void CEGPropertyGrid::SetOriginOffset(LONG nOffset)
{
	m_Origin.y = nOffset;
}

	
void CEGPropertyGrid::UpdatedItems()
{
	if (!IsWindow(m_hWnd))
		return;
...
LONG CEGPropertyGrid::GetColumn()
{
	return m_Origin.x;
}


void CEGPropertyGrid::SetColumn(LONG nColumn)
{
	CRect rc;
...
void CEGPropertyGrid::SetFocusedItem(CEGPropertyGridItem* pItem)
{
	m_pFocus = pItem;
...
BOOL CEGPropertyGrid::IsItemVisible(CEGPropertyGridItem* pItem)
{
	if (!pItem)
		return FALSE;

	for (CEGPropertyGridItem* pNext = m_pVisbleList; pNext; pNext = pNext->GetNextVisible())
	{
		if (pNext==pItem)
			return TRUE;
	}
...
LONG CEGPropertyGrid::HitTest(const POINT& pt)
{
	POINT p = pt;
...
CEGPropertyGridItem* CEGPropertyGrid::FindItem(const POINT& pt)
{
	CEGPropertyGridItem* pItem;
...
CEGPropertyGridItem* CEGPropertyGrid::FindItem(UINT nCtrlID)
{
	s_pFound = NULL;
...
BOOL CALLBACK CEGPropertyGrid::EnumFindItem(CEGPropertyGrid*, CEGPropertyGridItem* pItem, LPARAM lParam)
{
	ASSERT(pItem!=NULL);
...
BOOL CEGPropertyGrid::IsDisableInput()
{
	return m_bDisableInput;
}


void CEGPropertyGrid::DisableInput(BOOL bDisable)
{
...
void CEGPropertyGrid::SelectItems(CEGPropertyGridItem* pItem, BOOL bSelect)
{
	if (!pItem)
		pItem = &m_Root;
...
void CEGPropertyGrid::UpdateMoveAllItems()
{
...
BOOL CALLBACK CEGPropertyGrid::EnumSelectAll(CEGPropertyGrid*, CEGPropertyGridItem* pItem, LPARAM lParam)
{
	if (!pItem)
		return FALSE;
...
BOOL CALLBACK CEGPropertyGrid::EnumRefreshAll(CEGPropertyGrid*, CEGPropertyGridItem* pItem, LPARAM)
{
	if (!pItem)
		return FALSE;
...
BOOL CALLBACK CEGPropertyGrid::EnumMoveAll(CEGPropertyGrid*, CEGPropertyGridItem* pItem, LPARAM)
{
	if (!pItem)
		return FALSE;
...
LRESULT CEGPropertyGrid::SendNotify(UINT nNotifyCode, CEGPropertyGridItem* pItem)
{
	if (!IsWindow(m_hWnd))
		return 0L;
	...
	if (lpnm)
	{
		//UINT id = (UINT)::GetMenu(m_hWnd);#WARNING
...
void CEGPropertyGrid::OnEnable(BOOL bEnable) 
{
...
void CEGPropertyGrid::OnSysColorChange() 
{
...
BOOL CEGPropertyGrid::IsSingleSelection()
{
