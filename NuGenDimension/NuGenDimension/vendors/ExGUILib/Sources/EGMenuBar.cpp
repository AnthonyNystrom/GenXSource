...
//////////////////////////////////////////////////////////////////////
// Construction/Destruction CEGBarItem
//////////////////////////////////////////////////////////////////////

CEGBarItem::CEGBarItem()
: m_nSyncFlag(0),
m_nState(0)
{
}

CRect CEGBarItem::GetRect(BOOL bVertical) const
{
  if(bVertical)
  {
...
int CEGBarItem::HitTest(CPoint point,BOOL bVertical) const
{
  return GetRect(bVertical).PtInRect(point);
}
//////////////////////////////////////////////////////////////////////
// Construction/Destruction CEGMenuBarItem
//////////////////////////////////////////////////////////////////////
IMPLEMENT_DYNAMIC( CEGMenuBarItem, CEGBarItem )

CEGMenuBarItem::CEGMenuBarItem(HMENU hMenu, UINT nIndex)
: m_hMenu(hMenu),
m_nIndex(nIndex)
{
}

HMENU CEGMenuBarItem::GetMenu(HWND hWndOwner)
{
  UNREFERENCED_PARAMETER(hWndOwner);
  return GetSubMenu(m_hMenu,m_nIndex);
}

void CEGMenuBarItem::DrawBarItem(HWND hWndOwner, HDC hDC)
{
...
UINT CEGMenuBarItem::GetMenuItemID()
{
  return ::GetMenuItemID(m_hMenu,m_nIndex);
}

//////////////////////////////////////////////////////////////////////
// Construction/Destruction CEGIconBarItem
//////////////////////////////////////////////////////////////////////
CEGIconBarItem::CEGIconBarItem(HBITMAP hBitmap,UINT uCommandID)
: m_hBitmap(hBitmap),
...
CRect CEGIconBarItem::GetRect(BOOL bVertical) const
{
  if(bVertical)
  {
...
UINT CEGIconBarItem::GetMenuItemID()
{
  return m_uCommandID;
}
...
HMENU CEGIconBarItem::GetMenu(HWND hWndOwner)
{
...
void CEGIconBarItem::DrawSpecialChar(HDC hDC, LPCRECT pRect, TCHAR Sign, BOOL bBold, BOOL bDisabled)
{
...
void CEGIconBarItem::PaintHotButton(HDC hDC)
{ 
...
void CEGIconBarItem::DrawBarItem(HWND hWndOwner, HDC hDC)
{
  PaintHotButton(hDC);
  ...
  //switch((DWORD)m_hBitmap)#WARNING
  switch((DWORD_PTR)m_hBitmap)
  {
  case HBMMENU_SYSTEM:
    {
      rect.OffsetRect(2,2);
      ...
      if(hMDIClient)
      {
        //HICON hIcon = (HICON)GetClassLong( hMDIClient,GCL_HICONSM);#WARNING
		DWORD_PTR hIcon = GetClassLong( hMDIClient,GCL_HICONSM);
        if(hIcon)
        {
...
//////////////////////////////////////////////////////////////////////
// Construction/Destruction CEGMenuBar
//////////////////////////////////////////////////////////////////////

IMPLEMENT_DYNAMIC(CEGMenuBar, CControlBar)

// Special messages for handling menu
const UINT CEGMenuBar::WM_REMOVEMENU = ::RegisterWindowMessage(_T("CEGMenuBar::WM_REMOVEMENU"));
const UINT CEGMenuBar::WM_GETMENU = ::RegisterWindowMessage(_T("CEGMenuBar::WM_GETMENU"));

CEGMenuBar::CEGMenuBar()
...
BOOL CEGMenuBar::Create(CWnd* pParentWnd, DWORD dwStyle, UINT nID)
{
  return CreateEx(pParentWnd, 0, dwStyle,CRect(m_cxLeftBorder, m_cyTopBorder, m_cxRightBorder, m_cyBottomBorder), nID);
}

BOOL CEGMenuBar::CreateEx(CWnd* pParentWnd, DWORD dwCtrlStyle, DWORD dwStyle, CRect rcBorders, UINT nID)
{
...
CSize CEGMenuBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
...
CSize CEGMenuBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
  if ((nLength == -1) &&
    !(dwMode & LM_MRUWIDTH) &&
    !(dwMode & LM_COMMIT) &&
...
CSize CEGMenuBar::CalcLayout(DWORD dwMode, int nLength)
{
  ASSERT_VALID(this);
...
void CEGMenuBar::CorrectItemLayout(MBITEM* pData, int nFrom, int nTo, int nWidth)
{
...
void CEGMenuBar::UpdateItemLayout(MBITEM* pData, int nCount)
{
  int i;
  for (i = 0; i < nCount; i++)
  {
    CEGBarItem* pItem = m_NewBarItemList[i];
...
CSize CEGMenuBar::CalcSize(MBITEM* pData, int nCount)
{
  ASSERT(nCount > 0);
  CPoint cur(0,0);
  CSize sizeResult(0,0);
...
void CEGMenuBar::SizeMenuBar(MBITEM* pData, int nCount, int nLength, BOOL bVert)
{
...
int CEGMenuBar::WrapMenuBar(MBITEM* pData, int nCount, int nWidth)
{
  int nResult = 0;
  CPoint pos(0,0);
  int nMax_y=0;
...
int CEGMenuBar::GetClipBoxLength(BOOL bHorz)
{
...
BEGIN_MESSAGE_MAP(CEGMenuBar, CControlBar)
  ON_WM_CREATE()
  ON_WM_DESTROY()
  ON_WM_TIMER()
  ON_WM_NCPAINT()
...
CTypedPtrMap<CMapPtrToPtr,HWND,CEGMenuBar*> CEGMenuBar::m_NewMenuBar;

LRESULT CALLBACK CEGMenuBar::FrmWindowProc(HWND hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
{
#ifdef _USRDLL
...
LRESULT CALLBACK CEGMenuBar::CilWindowProc(HWND hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
{
#ifdef _USRDLL
  // If this is a DLL, need to set up MFC state
  AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
LRESULT CEGMenuBar::FrameWindowProc(HWND hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
{
  switch(nMsg)
  {
  case WM_DESTROY:
  ...
  case WM_MEASUREITEM:
    if(!CEGMenu::OnMeasureItem(GetCurrentMessage()))
    {
      ...
      if(pMeasureItem)
      {
        CMenu* pMenu = CEGMenu::FindPopupMenuFromID(m_hMenuBar,pMeasureItem->itemID);
        if(pMenu)
        {
  ...
  case WM_INITMENUPOPUP:
    ...
    //
    {
      ...
      //CEGMenu::OnInitMenuPopup(hWnd,CMenu::FromHandle((HMENU)wParam),(UINT)LOWORD(lParam), (BOOL)HIWORD(lParam));
  ...
  case WM_MENUCHAR:
    {
      ...
      CEGMenu* pMenu = DYNAMIC_DOWNCAST(CEGMenu,CMenu::FromHandle((HMENU)lParam));
      if( pMenu )
      {
        LRESULT lr = CEGMenu::FindKeyboardShortcut(nChar, nFlags, pMenu);
        if (lr!=0)
        {
          return lr;
        }
...
LRESULT CEGMenuBar::FrameDefault()
{
...
LRESULT CEGMenuBar::ChildWindowProc(HWND hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
{
  switch(nMsg)
  {
  case WM_DESTROY:
...
LRESULT CEGMenuBar::ChildDefault()
{
  // MFC stores current MSG in thread state
  MSG& curMsg = AfxGetThreadState()->m_lastSentMsg;
...
// CEGMenuBar message handlers
void CEGMenuBar::OnUpdateCmdUI(CFrameWnd* /*pTarget*/, BOOL /*bDisableIfNoHndler*/)
{
...
int CEGMenuBar::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CEGMenuBar::OnDestroy()
{
  CControlBar::OnDestroy();
  DeleteNewBarItemList();
}

void CEGMenuBar::DeleteNewBarItemList()
{
...
int CEGMenuBar::HitTest(LPPOINT ppt )const
{
...
void CEGMenuBar::OnMDISetMenu(HMENU hNewMenu)
{
  ...
  for (int iItem = 0; iItem < (int)nItems; iItem++)
  {
    ...
    if(GetMenuItemInfo(m_hMenuBar,iItem,TRUE,&MenuItemInfo))
    {
      ...
      {
        m_NewBarItemList.Add(new CEGIconBarItem((HBITMAP)MenuItemInfo.dwTypeData,MenuItemInfo.wID));
      }
      else
      {
      CEGMenuBarItem* pItem = new CEGMenuBarItem(m_hMenuBar,iItem);
  ...
  if(/*m_bMDIMaximized*/FALSE)
  {
		if( WS_THICKFRAME & ::GetWindowLong( hwndChild, GWL_STYLE ) ) {
	    m_NewBarItemList.InsertAt(0,new CEGIconBarItem(HBMMENU_SYSTEM,UINT(-1)));
		
	    m_NewBarItemList.Add(new CEGIconBarItem(HBMMENU_MBAR_MINIMIZE,SC_MINIMIZE));
		  m_NewBarItemList.Add(new CEGIconBarItem(HBMMENU_MBAR_RESTORE,SC_RESTORE));
			m_NewBarItemList.Add(new CEGIconBarItem(HBMMENU_MBAR_CLOSE,SC_CLOSE));
	  
			CEGBarItem* pItem = m_NewBarItemList[m_NewBarItemList.GetSize()-3];
		  pItem->m_nState |= ODS_RIGHTJUSTIFY;
		}
  }
  RefreshBar();
...
void CEGMenuBar::RefreshBar()
{
	// recalc items sizes... by me
	UINT nMenuItem = 0;
	UINT nItems = (UINT)m_NewBarItemList.GetCount();
	for (int iItem = 0; iItem < (int)nItems; iItem++) {
		CEGBarItem* pItem = m_NewBarItemList.GetAt( iItem );
		if ( !pItem->IsKindOf( RUNTIME_CLASS( CEGMenuBarItem ) ) ) 
			continue;
		MENUITEMINFO MenuItemInfo = {0};
		MenuItemInfo.cbSize = sizeof(MenuItemInfo);
		MenuItemInfo.fMask = MIIM_DATA|MIIM_TYPE|MIIM_ID;
		if(GetMenuItemInfo(m_hMenuBar, nMenuItem++, TRUE, &MenuItemInfo)) {
			if( !(MenuItemInfo.fType & MF_BITMAP) ) {
				CEGMenuBarItem* pItem = (CEGMenuBarItem*)m_NewBarItemList.GetAt( iItem );
				if(MenuItemInfo.fType & MF_OWNERDRAW) {
					MEASUREITEMSTRUCT measureItem = {0};
					measureItem.CtlType = ODT_MENU;
					measureItem.itemID = MenuItemInfo.wID;
...
void CEGMenuBar::OnPaint()
{
  ...
  {
    CEGBarItem *pBarItem = m_NewBarItemList[n];
...
void CEGMenuBar::DrawGripper(CDC* pDC, const CRect& rect)
{
...
#if _MFC_VER < 0x0800  
  afx_msg UINT CEGMenuBar::OnNcHitTest(CPoint point)
  {
#else
  afx_msg LRESULT CEGMenuBar::OnNcHitTest(CPoint point)
  {
#endif
  return HTCLIENT;
}

void CEGMenuBar::OnNcPaint()
{
  EraseNonClient();
}

void CEGMenuBar::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp)
{
...
void CEGMenuBar::EraseNonClient()
{
...
void CEGMenuBar::DrawBorders(CDC*, CRect&)
{
  return;
}

BOOL CEGMenuBar::OnEraseBkgnd(CDC* pDC)
{
  ...
  else
  {
    ...
    GetWindowRect(clientRect);
    ScreenToClient(clientRect);
    pDC->FillSolidRect(clientRect,CEGMenu::GetMenuBarColor());//GetSysColor(COLOR_3DLIGHT));
  }
  return true;
}
int CEGMenuBar::GetNextItem(int nIndex, BOOL bNext)
{
  int nCount = GetItemCount();
  int nTest = nCount;
  while(nIndex<0)
...
CEGMenuBar* CEGMenuBar::g_pMenuBar = NULL;
HHOOK CEGMenuBar::g_hMsgHook = NULL;

LRESULT CALLBACK CEGMenuBar::MenuInputFilter(int code, WPARAM wParam, LPARAM lParam)
{
  return (code==MSGF_MENU && g_pMenuBar && g_pMenuBar->OnMenuInput( *((MSG*)lParam) )) ? TRUE : CallNextHookEx(g_hMsgHook, code, wParam, lParam);
}

BOOL CEGMenuBar::OnMenuInput(MSG msg)
{
  ASSERT_VALID(this);
...
void CEGMenuBar::SetSel(int nIndex, ETrackingState eState)
{
  ...
  if(nIndex!=m_nActualSelIndex)
  {
    ...
    {
      CEGBarItem *pItem = m_NewBarItemList[m_nActualSelIndex];
...
UINT CEGMenuBar::EnableTimer(BOOL bEnable)
{
  if(bEnable)
  {
    if(!m_TimerID)
...
BOOL CEGMenuBar::TranslateFrameMessage(MSG* pMsg)
{
  ASSERT_VALID(this);
  ASSERT(pMsg);
  ...
  { // Alt key presed?
    ...
    else if ((nMsg == WM_SYSKEYDOWN || nMsg == WM_KEYDOWN))
    {
          ...
          case VK_RETURN:
            {
              ...
              {
                CEGBarItem *pItem = m_NewBarItemList[m_nCurIndex];
      ...
      {
        LRESULT lr = CEGMenu::FindKeyboardShortcut(vkey,0, CMenu::FromHandle(m_hMenuBar));
        if (lr)
        {
          if(m_bMDIMaximized)
          {
...
void CEGMenuBar::OnTimer(UINT nIDEvent)
{
...
void CEGMenuBar::OnMouseMove(UINT nFlags, CPoint point)
{
  if(m_ptMouse!=point)
  {
    CFrameWnd* pFrame = GetDockingFrame();
...
void CEGMenuBar::OnLButtonDown(UINT nFlags, CPoint point)
{
  m_nMouseIndex = -1;
...
BOOL CEGMenuBar::ShowMenu(int nIndex)
{
  if(m_bLoop)
  {
    return FALSE;
  ...
  {
    CEGBarItem *pItem = m_NewBarItemList[m_nCurIndex=nIndex];
    do
    {
...
void CEGMenuBar::OnLButtonUp(UINT nFlags, CPoint point)
{
  int nIndex = HitTest(&point);
  if(nIndex!=-1)
  {
    CEGBarItem *pItem = m_NewBarItemList[nIndex];
    if(!::IsMenu(pItem->GetMenu(m_hWndOwner)))
    {
      SetSel(m_nCurIndex,TR_NONE);
      ::SendMessage(m_hWndOwner,WM_COMMAND, (WPARAM)pItem->GetMenuItemID(), (LPARAM)m_hWnd);
