...
long CxImage::Draw(HDC hdc, long x, long y, long cx, long cy, RECT* pClipRect)
{
	...
	if (GetType()==CXIMAGE_FORMAT_WMF && m_hMeta)
	{
		...
		PlayEnhMetaFile(hdc,m_hMeta,&paintbox);
		goto nextLab;
	}
#endif
...
long CxImage::Stretch(HDC hdc, long xoffset, long yoffset, long xsize, long ysize, DWORD dwRop)
{
	...
	{
		RECT paintbox;
		paintbox.left = xoffset;
		paintbox.right = xoffset+xsize;
		paintbox.top = yoffset;
		paintbox.bottom = yoffset+ysize;
		PlayEnhMetaFile(hdc,m_hMeta,&paintbox);
		return 1;
	}
#endif
