...
bool CxImageWMF::Decode(CxFile *hFile, long nForceWidth, long nForceHeight)
{
	...
	METAFILEHEADER	mfh;
	m_hMeta = ConvertWmfFiletoEmf(hFile, &mfh);
	
	if (m_hMeta) {	// ok, it's a WMF
...
/////////////////////////////////////////////////////////////////////
		...
		ENHMETAHEADER	emh;
		m_hMeta = ConvertEmfFiletoEmf(hFile, &emh);
	...
	if (hDC){
		if (hBitmap){
			...
			int iEntries = GetEnhMetaFilePaletteEntries(m_hMeta, 0, NULL);
			...
			if (iEntries) { 
				...
				GetEnhMetaFilePaletteEntries(m_hMeta, iEntries, (PPALETTEENTRY)pjTmp); 
			...
			BOOL bRet = PlayEnhMetaFile(hDC,	// handle to a device context 
									m_hMeta,	// handle to an enhanced metafile  
									&rc); 	// pointer to bounding rectangle

			SelectObject(hDC, hBitmapOld);
			//DeleteEnhMetaFile(m_hMeta);	// we are done with this one
