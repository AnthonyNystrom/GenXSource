...
int CNumEdit::IsValid() const
{
	...
	{
		str.Empty();
		if (res & OUT_OF_RANGE) str += _T("Значение не в пределах.\n");
		if (res & INVALID_CHAR) str += _T("Символ д.б. числом\n");
...
int CNumEdit::IsValid(const CString &str) const
{
	...
	if (m_Verbose && (res != VALID) && (res != MINUS_PLUS))
	{
		CString msg;
		msg.Empty();
		if (res & OUT_OF_RANGE) msg += _T("Значение не в пределах.\n");
		if (res & INVALID_CHAR) msg += _T("Символ д.б. числом\n");
		AfxMessageBox(str, MB_OK | MB_ICONSTOP);
	}
	return res;
}
