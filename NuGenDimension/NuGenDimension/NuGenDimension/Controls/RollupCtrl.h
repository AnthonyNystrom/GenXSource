...
// CRollupCtrl structures and defines
	...
	struct RC_PAGEINFO {
		CString		cstrCaption;
		CSize       CaptionSizes;
		IBaseInterfaceOfGetDialogs*			pwndTemplate;
		CButton*	pwndButton;
	...
	#define	RC_PGBUTTONHEIGHT				15 //18
...
public:
	...
	int		InsertPage(const char* caption, IBaseInterfaceOfGetDialogs* pwndTemplate, 
		bool withRadio,BOOL bAutoDestroyTpl=TRUE, int idx=-1);	//Return page zero-based index

	void    EnableRadio(unsigned int,bool);
	void    SetActiveRadio(unsigned int, bool scroll=true);

	
	bool	RemovePage(int idx);	//idx is a zero-based index
	bool	RemovePage(IBaseInterfaceOfGetDialogs* pg);

	bool    RenamePage(unsigned int ind, const char* lab);
...
protected:
	...
	int		_InsertPage(const char* caption, IBaseInterfaceOfGetDialogs* dlg, 
