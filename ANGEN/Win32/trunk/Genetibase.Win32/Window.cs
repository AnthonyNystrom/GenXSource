...
namespace Genetibase.WinApi
{
	...
	public static class Window
	{
		...
		public static FormBorderStyle GetBorderStyle(IntPtr hWnd)
		{
			...
			Int32 style = GetStyle(hWnd);
			Int32 exStyle = GetExStyle(hWnd);
		...
		/// <paramref name="hWnd"/> does not represent a window.
		/// </exception>
		public static Int32 GetExStyle(IntPtr hWnd)
		{
			if (!User32.IsWindow(hWnd))
			{
				throw new NuGenInvalidHWndException(hWnd);
		...
		/// <paramref name="hWnd"/> does not represent a window.
		/// </exception>
		public static Int32 GetStyle(IntPtr hWnd)
		{
			if (!User32.IsWindow(hWnd))
			{
				throw new NuGenInvalidHWndException(hWnd);
		...
		/// <paramref name="hWnd"/> does not represent a window.
		/// </exception>
		public static Boolean IsRightToLeft(IntPtr hWnd)
		{
			if (!User32.IsWindow(hWnd))
			{
				throw new NuGenInvalidHWndException(hWnd);
			...
			Int32 exStyle = Window.GetExStyle(hWnd);
