...
namespace Genetibase.WinApi
{
	...
	public static class User32
	{
		...
		/// <param name="dwOldKey">Pointer to a COLORREF value that receives the transparency color key to be used when composing the layered window. All pixels painted by the window in this color will be transparent. This can be 0 if the argument is not needed.</param>
		/// <param name="bOldAlpha">Pointer to a BYTE that receives the Alpha value used to describe the opacity of the layered window. Similar to the SourceConstantAlpha member of the BLENDFUNCTION structure. When the variable referred to by pbAlpha is 0, the window is completely transparent. When the variable referred to by pbAlpha is 255, the window is opaque. This can be 0 if the argument is not needed.</param>
		/// <param name="dwOldFlags">
		/// <para>The layering flag can be one or more of the following values.</para>
		/// <para>LWA_ALPHA - Use pbAlpha to determine the opacity of the layered window.</para>
		/// <para>LWA_COLORKEY - Use pcrKey as the transparency color.</para>
		/// </param>
		/// <returns>
		/// If the function succeeds, the return value is <see langword="true"/>.
		...
		[DllImport(DLL)]
		[return: MarshalAs(UnmanagedType.Bool)]
		public static extern Boolean GetLayeredWindowAttributes(IntPtr hWnd, out Int32 dwOldKey, out Byte bOldAlpha, out Int32 dwOldFlags);
		...
		/// <summary>
		/// retrieves the update region of a window by copying it into the specified region. The coordinates of the update region are relative to the upper-left corner of the window (that is, they are client coordinates).
		/// </summary>
		...
		/// <param name="bErase">Specifies whether the window background should be erased and whether nonclient areas of child windows should be drawn. If this parameter is <see langword="false"/>, no drawing is done.</param>
		/// <returns>
		...
		/// <para>SIMPLEREGION - Region is a single rectangle.</para>
		/// </returns>
		[DllImport(DLL)]
		public static extern Int32 GetUpdateRgn(IntPtr hWnd, IntPtr hRgn, Boolean bErase);
		...
		/// </summary>
		/// <param name="hWnd">Handle to the layered window.</param>
		/// <param name="crKey">COLORREF structure that specifies the transparency color key to be used when composing the layered window. All pixels painted by the window in this color will be transparent. To generate a COLORREF, use the RGB macro.</param>
		...
		/// <param name="dwFlags">
		/// <para>Specifies an action to take. This parameter can be one or more of the following values.</para>
		/// <para>LWA_COLORKEY - Use crKey as the transparency color.</para>
		/// <para>-or-</para>
		/// <para>LWA_ALPHA - Use bAlpha to determine the opacity of the layered window.</para>
		/// </param>
		[DllImport(DLL)]
		public static extern void SetLayeredWindowAttributes(IntPtr hWnd, Int32 crKey, Byte bAlpha, Int32 dwFlags);
