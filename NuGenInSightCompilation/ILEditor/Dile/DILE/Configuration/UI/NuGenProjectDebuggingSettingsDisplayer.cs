...
{
	public class NuGenProjectDebuggingSettingsDisplayer : NuGenBaseSettingsDisplayer
	{
		...
		private void FillDebugEventsList()
		{
			...
			foreach(string suspendableDebugEventName in Enum.GetNames(typeof(SuspendableDebugEvent)))
			{
				...
				{
					DebugEventsList.Items.Add(suspendableDebugEvent, NuGenProject.Instance.SuspendOnDebugEvent(suspendableDebugEvent));
				}
			}
		...
		{
			NuGenProject.Instance.SuspendingDebugEvents.Clear();

			foreach (SuspendableDebugEvent suspendableDebugEvent in DebugEventsList.CheckedItems)
			{
				NuGenProject.Instance.SuspendingDebugEvents.Add(suspendableDebugEvent);
			}
		}
