...
{
	public class NuGenProjectStartupSettingsDisplayer : NuGenBaseSettingsDisplayer
	{
		...
		protected override void CreateControls(TableLayoutPanel panel)
		{
			...
			StartAssembly = CreateRadioButton(panel, zeroPadding, "Start selected assembly", (NuGenProject.Instance.StartMode == ProjectStartMode.StartAssembly));
			StartAssemblyArguments = CreateTextBox(panel, leftPadding, "Arguments:", NuGenProject.Instance.AssemblyArguments);
			Button browseAssemblyWorkingDirectory = CreateBrowseButton();
			StartAssemblyWorkingDirectory = CreateTextBox(panel, leftPadding, "Working directory:", NuGenProject.Instance.AssemblyWorkingDirectory, browseAssemblyWorkingDirectory);
			browseAssemblyWorkingDirectory.Tag = StartAssemblyWorkingDirectory;

			StartProgram = CreateRadioButton(panel, zeroPadding, "Start program", (NuGenProject.Instance.StartMode == ProjectStartMode.StartProgram));
			Button browseExecutable = CreateBrowseButton();
			StartProgramExecutable = CreateTextBox(panel, leftPadding, "Executable:", NuGenProject.Instance.ProgramExecutable, browseExecutable);
			browseExecutable.Tag = StartProgramExecutable;
			StartProgramArguments = CreateTextBox(panel, leftPadding, "Arguments:", NuGenProject.Instance.ProgramArguments);
			Button browseProgramWorkingDirectory = CreateBrowseButton();
			StartProgramWorkingDirectory = CreateTextBox(panel, leftPadding, "Working directory:", NuGenProject.Instance.ProgramWorkingDirectory, browseProgramWorkingDirectory);
		...
		public override void ReadSettings()
		{
			...
			{
				NuGenProject.Instance.StartMode = ProjectStartMode.StartAssembly;
			}
			else if (StartProgram.Checked)
			{
				NuGenProject.Instance.StartMode = ProjectStartMode.StartProgram;
			}
			#region ASP.NET Debugging
			//else
			//{
			...
			NuGenProject.Instance.AssemblyArguments = StartAssemblyArguments.Text;
			NuGenProject.Instance.AssemblyWorkingDirectory = StartAssemblyWorkingDirectory.Text;
			NuGenProject.Instance.ProgramExecutable = StartProgramExecutable.Text;
			NuGenProject.Instance.ProgramArguments = StartProgramArguments.Text;
			NuGenProject.Instance.ProgramWorkingDirectory = StartProgramWorkingDirectory.Text;
			#region ASP.NET Debugging
